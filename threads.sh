#!/usr/bin/env bash
set -euo pipefail

# =========================
# Config (defaults; overridable by flags)
# =========================
INPUT_CSV="/mnt/nvme1/cmbenello/data/sf500/lineitem.csv"     # CSV source
KVBIN_OUT="/mnt/nvme1/cmbenello/data/sf500/lineitem.kvbin"    # KVBin source (auto-built if needed)
KVBIN_OUT="/tank/local/cb/sf500/lineitem.kvbin"    # KVBin source (auto-built if needed)

BENCHMARK_RUNS=3
WARMUP_RUNS=1

# SF500 KVBin: keys are all numbers 1..12 internally (handled by the binary)
# Merge fan-in during merge phase
MERGE_FANIN=1024

# Write an index entry every N records when converting CSV -> KVBin
IDX_EVERY=100000

# Constant *per-thread* run size (MB)
RUN_SIZE_MB=314

# Sweep of thread counts
THREAD_SWEEP=(32 24 20 16 12 8 4 2 1)

# OVC variant controls
OVC_VARIANT="yes"         # no | yes | both

# Build control
SKIP_BUILD="no"            # yes | no

# =========================
# Styling
# =========================
RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; BLUE='\033[0;34m'; NC='\033[0m'

usage() {
  cat <<EOF
Usage: $0 [options]

Options:
  --ovc=no|yes|both          Run without OVC, with OVC, or both (default: both)
  --runs=N                   Benchmark runs (default: $BENCHMARK_RUNS)
  --warmup=N                 Warmup runs (default: $WARMUP_RUNS)
  --rs-mb=N                  Per-thread run size in MB (default: $RUN_SIZE_MB)
  --threads-sweep="a,b,c"    Thread-count sweep (default: 1..32)
  --csv=PATH                 CSV input path (default: $INPUT_CSV)
  --kvbin=PATH               KVBin path (default: $KVBIN_OUT)
  --fanin=N                  Merge fan-in (default: $MERGE_FANIN)
  --idx-every=N              csv2kvbin index stride (default: $IDX_EVERY)
  --skip-build               Use existing binaries; don't build
  -h|--help                  Show this help

Examples:
  $0 --ovc=both
  $0 --ovc=no --rs-mb=256 --threads-sweep="1,2,4,8,16,32"
EOF
}

# =========================
# Parse args
# =========================
for arg in "$@"; do
  case "$arg" in
    --ovc=*) OVC_VARIANT="${arg#*=}";;
    --runs=*) BENCHMARK_RUNS="${arg#*=}";;
    --warmup=*) WARMUP_RUNS="${arg#*=}";;
    --rs-mb=*) RUN_SIZE_MB="${arg#*=}";;
    --threads-sweep=*)
      IFS=',' read -r -a THREAD_SWEEP <<< "${arg#*=}"
      ;;
    --csv=*) INPUT_CSV="${arg#*=}";;
    --kvbin=*) KVBIN_OUT="${arg#*=}";;
    --fanin=*) MERGE_FANIN="${arg#*=}";;
    --idx-every=*) IDX_EVERY="${arg#*=}";;
    --skip-build) SKIP_BUILD="yes";;
    -h|--help) usage; exit 0;;
    *) echo -e "${RED}Unknown option: $arg${NC}"; usage; exit 1;;
  esac
done

echo -e "${BLUE}========================================================${NC}"
echo -e "${BLUE}  Lineitem Sort Benchmark: KVBin only (SF500)          ${NC}"
echo -e "${BLUE}  ovc=${OVC_VARIANT}, run-size-mb=${RUN_SIZE_MB}       ${NC}"
echo -e "${BLUE}========================================================${NC}\n"

# =========================
# Sanity checks
# =========================
if [[ ! -f "$INPUT_CSV" ]]; then
  echo -e "${RED}Warning: CSV input '$INPUT_CSV' not found (only needed if KVBin is missing).${NC}"
fi

# =========================
# Build
# =========================
if [[ "$SKIP_BUILD" != "yes" ]]; then
  echo -e "${YELLOW}Building binaries...${NC}"
  cargo build --release --example lineitem_benchmark_cli
  cargo build --release --example csv2kvbin || true  # if present
fi

BENCH_BIN="./target/release/examples/lineitem_benchmark_cli"
CSV2KVBIN="./target/release/examples/csv2kvbin"

if [[ ! -x "$BENCH_BIN" ]]; then
  echo -e "${RED}Error: benchmark binary missing at $BENCH_BIN${NC}"; exit 1
fi

# =========================
# Ensure KVBin exists
# =========================
if [[ ! -f "$KVBIN_OUT" ]]; then
  if [[ -x "$CSV2KVBIN" && -f "$INPUT_CSV" ]]; then
    echo -e "${YELLOW}No KVBin found. Converting CSV -> KVBin (+.idx)...${NC}"
    "$CSV2KVBIN" \
      --input  "$INPUT_CSV" \
      --output "$KVBIN_OUT" \
      --keys   "10,5,1" \
      --vals   "0,3,11" \
      --idx-every "$IDX_EVERY"
  else
    echo -e "${RED}KVBin '$KVBIN_OUT' missing and csv2kvbin example or CSV not available.${NC}"
    echo -e "${RED}Build or provide KVBin manually, or add the csv2kvbin example & CSV.${NC}"
    exit 1
  fi
fi

# =========================
# Results dir + helpers
# =========================
ts="$(date +%Y%m%d_%H%M%S)"
RESULTS_DIR="benchmark_results/kvbin_thread_sweep_${OVC_VARIANT}_${ts}"
mkdir -p "$RESULTS_DIR"
echo -e "${GREEN}Results directory: $RESULTS_DIR${NC}\n"

extract_metrics() {
  local output="$1"
  local label="$2"
  local logfile="$3"

  local sort_time=$(echo "$output" | grep -m1 "Sort completed in" | sed 's/.*Sort completed in \([0-9.]*\)s.*/\1/')
  local throughput=$(echo "$output" | grep -m1 "Throughput:" | sed 's/.*Throughput: \([0-9.]*\) MB\/s.*/\1/')
  local peak_memory=$(echo "$output" | grep -m1 "Peak memory usage:" | sed 's/.*Peak memory usage: \([0-9.]*\) MB.*/\1/')
  local total_io=$(echo "$output" | grep -m1 "Total I/O:" | sed 's/.*Total I/O: \([0-9.]*\) MB.*/\1/')
  local avg_key_size=$(echo "$output" | grep -m1 "Average key size:" | sed 's/.*Average key size: \([0-9.]*\) bytes.*/\1/')
  local avg_value_size=$(echo "$output" | grep -m1 "Average value size:" | sed 's/.*Average value size: \([0-9.]*\) bytes.*/\1/')

  echo -e "${YELLOW}=== $label ===${NC}"
  [ -n "${sort_time:-}" ]      && echo "  Sort time: ${sort_time}s"
  [ -n "${throughput:-}" ]     && echo "  Throughput: ${throughput} MB/s"
  [ -n "${peak_memory:-}" ]    && echo "  Peak memory: ${peak_memory} MB"
  [ -n "${total_io:-}" ]       && echo "  Total I/O: ${total_io} MB"
  [ -n "${avg_key_size:-}" ]   && echo "  Avg key size: ${avg_key_size} bytes"
  [ -n "${avg_value_size:-}" ] && echo "  Avg value size: ${avg_value_size} bytes"
  echo

  echo "$label,$sort_time,$throughput,$peak_memory,$total_io,$avg_key_size,$avg_value_size" >> "$RESULTS_DIR/summary.csv"
  echo "  Log: $logfile"
}

# CSV header for combined summary
echo "Label,Sort_Time_s,Throughput_MB_s,Peak_Memory_MB,Total_IO_MB,Avg_Key_Size,Avg_Value_Size" > "$RESULTS_DIR/summary.csv"

run_kvbin_variant() {
  local threads="$1"
  local with_ovc="$2"   # yes|no
  local tag="KVBin_$( [[ $with_ovc == yes ]] && echo OVC || echo NoOVC )_thr${threads}_rs${RUN_SIZE_MB}"
  local log="$RESULTS_DIR/${tag}.txt"

  echo -e "${BLUE}>>> KVBin mode: $( [[ $with_ovc == yes ]] && echo WITH || echo NO ) OVC (threads=${threads}, run-size-mb=${RUN_SIZE_MB})${NC}"

  "$BENCH_BIN" \
    -i "$KVBIN_OUT" \
    --benchmark-runs "$BENCHMARK_RUNS" \
    --warmup-runs "$WARMUP_RUNS" \
    $([[ $with_ovc == yes ]] && echo --ovc) \
    --run-gen-threads "$threads" \
    --merge-threads "$threads" \
    --run-size-mb "$RUN_SIZE_MB" \
    --merge-fanin "$MERGE_FANIN" 2>&1 | tee "$log"

  # Clean up warmup / temp files after warmup+run
  rm -rf warmup* 17* || true

  extract_metrics "$(cat "$log")" "$tag" "$log"
}

# =========================
# Sweep over threads
# =========================
for T in "${THREAD_SWEEP[@]}"; do
  echo -e "${BLUE}==============================${NC}"
  echo -e "${BLUE} Threads: ${T}                ${NC}"
  echo -e "${BLUE}==============================${NC}"

  case "$OVC_VARIANT" in
    no)   run_kvbin_variant "$T" "no" ;;
    yes)  run_kvbin_variant "$T" "yes" ;;
    both) run_kvbin_variant "$T" "no"; run_kvbin_variant "$T" "yes" ;;
    *) echo -e "${RED}Invalid --ovc value: $OVC_VARIANT${NC}"; exit 1;;
  esac
  echo
done

echo -e "${GREEN}Summary written to:${NC} $RESULTS_DIR/summary.csv"
echo "All logs in: $RESULTS_DIR"
