/// Generate a synthetic KVBin dataset with **frequency skew only** (no size skew).
///
/// # Dataset characteristics
///
/// A configurable fraction of rows (`--dup-frac`, default 50 %) share one heavy-hitter key
/// (`--heavy-key`, default 0).  Every row — heavy-hitter or not — carries the same payload
/// size (`--payload`, default 512 B).  There is no size skew whatsoever.
///
/// This dataset isolates **duplicate splitting** as the sole challenge for partition planners:
/// - Key-Only cannot split within a key → one partition absorbs 50 % of rows and bytes.
/// - Count-Balanced splits within the heavy key via the virtual order `(key, run_id, offset)`
///   and achieves equal byte volume because record sizes are uniform.
/// - Byte-Balanced degenerates to Count-Balanced (all weights are equal).
///
/// Contrast with `heavy_key`, which adds size skew on top of frequency skew, causing
/// Count-Balanced to fail as well.
///
/// Default parameters target ~200 GiB of raw data with ~407 M rows:
/// ```text
/// record = 4 + 8 + 4 + 512 = 528 B
/// 200 GiB / 528 B ≈ 406 720 387 rows
/// ```
///
/// # KVBin record format
///
...
struct Args {
    ...
    /// Default targets ~200 GiB: record = 4+8+4+512 = 528 B → 200 GiB / 528 ≈ 406 720 387 rows
...
fn append_record(buf: &mut Vec<u8>, key: u64, payload_len: usize, rng: &mut SplitMix64) {
    ...
    // from resize() would be wasted work.
    let start = buf.len();
