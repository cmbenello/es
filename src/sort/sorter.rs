...
use crate::rs::replacement_selection::{ReplacementSelectionSink, run_replacement_selection};
...
use crate::{
    MergeStats, RunGenerationStats, SortInput, SortOutput, SortStats, Sorter, TempDirInfo,
};
...
impl RunSummary for MergeableRun {
...
pub(crate) const ENTRY_METADATA_SIZE: usize = std::mem::size_of::<u32>() * 2;
...
impl RunWriterSink {
    ...
    fn into_parts(mut self) -> (Vec<MergeableRun>, Sketch<Vec<u8>>) {
...
impl ReplacementSelectionSink for RunWriterSink {
    fn start_run(&mut self) {
        ...
        let run = RunImpl::from_writer_with_indexing_interval(writer, self.run_indexing_interval)
...
pub(crate) fn append_record_to_buffer(
    ...
    if buffer.has_space(&key, &value) {
        ...
        if buffer.is_empty() {
            ...
            panic!(
                "Record of size {} bytes exceeds quicksort run size limit {} bytes",
                entry_size, run_size
            );
        }
        Err((key, value))
        ...
        match algorithm {
            RunGenerationAlgorithm::ReplacementSelection => {
                let worker = move |thread_id: usize,
                    ...
                    let mut sink = RunWriterSink::new(
                        run_writer,
                        run_indexing_interval,
                        sketch_size,
                        sketch_sampling_interval,
