    ...
    S: RunSink,
{
    ...
    loop {
        ...
        //   tree_heap     =  capacity × S   (S = size of Node<OVCKeyValueMM>)
        ...
        //   S            = size_of::<OVCKeyValueMM>() + size_of::<u32>()   (12 B)
        //   2S           = node_overhead    (24 B)
        //
        ...
        if !limit_adjusted && rs.buffer_len() > 0 && pending_record.is_some() && initial_limit > 0 {
            ...
            let node_overhead = // 2S
                2 * (std::mem::size_of::<OVCKeyValueMM>() + std::mem::size_of::<u32>());
            let ideal_limit =
...
mod tests {
    ...
    fn test_adaptive_limit_grows_past_initial_50_percent() {
        ...
        //   node_overhead = 2 × (8 + 4) = 24                                   (2S)
        //
        // memory_limit = 2 MiB:
        //   initial_limit = 1 MiB  → 16 extents → 1024 records   (50 %)
        //   ideal_limit   = 2 MiB × 1024 / 1048
        //                 = 2 049 125 → rounds to 31 extents → 1984 records
        //
        ...
        let scanner: ReplacementScanner = Box::new(data.into_iter());
        let mut sink = CollectingSink::new();
        run_replacement_selection_ovc_mm(scanner, &mut sink, memory_limit);
