...
fn test_basic_functionality() {
    sorter_behavior::basic_sort(|| ExternalSorter::new(1, 512 * 1024, 1, 10000, 100, test_dir()));
}

#[test]
fn test_empty_input() {
    sorter_behavior::empty_input(|| ExternalSorter::new(2, 512 * 1024, 2, 10000, 100, test_dir()));
}

#[test]
fn test_single_element() {
    sorter_behavior::single_element(|| {
        ExternalSorter::new(2, 512 * 1024, 2, 10000, 100, test_dir())
    });
}
...
fn test_large_dataset_with_threading() {
    sorter_behavior::large_dataset_with_threads(|threads| {
        ExternalSorter::new(threads, 1024 * 1024, threads, 10000, 100, test_dir())
    });
}
...
fn test_run_generation_stats_with_small_buffer() {
    sorter_behavior::run_generation_stats_small_buffer(|| {
        ExternalSorter::new(1, 128 * 1024, 1, 10000, 100, test_dir())
    });
}
...
fn test_variable_sized_data() {
    sorter_behavior::variable_sized_data(|| {
        ExternalSorter::new(2, 1024 * 1024, 2, 10000, 100, test_dir())
    });
}
...
fn test_large_values() {
    sorter_behavior::large_values(|| ExternalSorter::new(2, 512 * 1024, 2, 10000, 100, test_dir()));
}
...
#[test]
fn test_binary_keys() {
    sorter_behavior::binary_keys(|| ExternalSorter::new(2, 512 * 1024, 2, 10000, 100, test_dir()));
}
