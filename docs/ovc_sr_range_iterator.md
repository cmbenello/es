...
This document explains how range iteration works in `RunWithOVCSR` (OVC sparse-run format), including the composite key ordering and how the sparse index is used to find the start point.

**Overview**
- Each run has a `run_id` assigned at creation time.
- Each record is ordered by its file offset within the run.
- The iterator compares by a composite key `(key, run_id, file_offset)`.
- Range iteration returns records whose composite keys satisfy: `lower_bound <= (key, run_id, offset) < upper_bound`.

**Bounds Format**
- Bounds are passed as `Option<CompositeBound>`:
  - `key: &[u8]`
  - `run_id: u32`
...
- `actual_start` (run start): absolute file offset where this run begins (`start_bytes`).

**What Actually Happens At Start**
1. Binary search sparse index with `lower_bound`.
2. Pick the last entry with `entry < lower_bound` (by `(key, run_id, offset)` order).
3. Seek to `aligned_offset` and skip `skip_bytes` to reach `start_offset`.
4. Initialize:
   - `prev_key = start_key`
5. Begin scanning forward in file order, comparing `(key, run_id, offset)` to bounds.

**Summary Of Key Roles**
- `lower_bound`: the logical start of the scan (composite order).
- `start_key/start_suffix`: a *nearby* position used to begin scanning.
- `actual_start`/`start_offset`: the byte position where scanning actually begins.
...
- It returns the last entry with `entry < lower_bound` in composite order.
...
- `current_offset`: derived from the current byte position (`bytes_read - actual_start`).
- `prev_key`: initialized to the sparse index key (full key).
- `bytes_read`: tracks file position, with direct-I/O alignment handled by `align_down`.
...
**Composite Comparison Semantics**
...
- Bounds are always composite tuples (unless empty).
...
**Why This Works**
- The composite key creates a total order among duplicates.
...
- If a lower bound lands in the middle of a run, the iterator will skip until the bound is met using composite comparisons.
