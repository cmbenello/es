...
# Thread sweeps
RUN_GEN_THREADS=(48 40 32 24 16 8 4 2 1)
MERGE_THREADS=(48 40 32 24 16 8 4 2 1)

# >>> Fixed total run-generation memory budgets (MB) <<<
# Put just 2048 here if you only want 2 GB:
MEM_BUDGETS_MB=(512 1024 256)

# Bench config
BENCHMARK_RUNS=1
WARMUP_RUNS=1
MERGE_FANIN=512
TEMP_DIR="${ES_ROOT}" # where temp files go
...
# Ensure KVBin exists (must already be built)
if [[ ! -f "$KVBIN" ]]; then
...
echo "Config,MemBudgetMB,RunSizePerThreadMB,RunGenThreads,MergeThreads,Sort_Time_s,Throughput_MB_s,Peak_Memory_MB,Total_IO_MB,Avg_Key_Size,Avg_Value_Size" \
  > "$SUMMARY_CSV"
...
extract_metrics() {
  ...
  echo "${label},${mem_budget},${run_size_per_thread},${rg},${mt},${sort_time},${throughput},${peak_memory},${total_io},${avg_key_size},${avg_value_size}" \
    >> "$SUMMARY_CSV"
...
run_one_config() {
  ...
  # per-thread run size so that total run-gen memory ~= mem_budget_mb
  # (float with 2 decimal places)
  local run_size_per_thread
  run_size_per_thread=$(awk -v total="$mem_budget_mb" -v thr="$rg" 'BEGIN { printf "%.2f", total/thr }')

  local label="mem${mem_budget_mb}_rg${rg}_mg${mt}"
  local log="${RESULTS_DIR}/${label}.txt"

  echo -e "${BLUE}>>> START Config: mem_budget=${mem_budget_mb} MB, run_gen_threads=${rg}, merge_threads=${mt}, run_size_per_thread=${run_size_per_thread} MB (OVC ON)${NC}"
  ...
  if ! "$BENCH_BIN" \
      ...
      --run-gen-threads "$rg" \
      --merge-threads "$mt" \
      --run-size-mb "$run_size_per_thread" \
  ...
  extract_metrics "$(cat "$log")" "$label" "$mem_budget_mb" "$run_size_per_thread" "$rg" "$mt"
  echo -e "${BLUE}>>> DONE  Config: ${label}${NC}"
  return 0
}

########################################
# Sweep: (mem_budget × run_gen_threads × merge_threads) with OVC
########################################
for mem in "${MEM_BUDGETS_MB[@]}"; do
  echo -e "${GREEN}=== Total run-gen memory budget: ${mem} MB ===${NC}"
