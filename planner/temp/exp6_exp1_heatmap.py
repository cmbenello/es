#!/usr/bin/env python3
"""
Create combined heatmaps for Exp6 and Exp1:
...
def parse_exp1_log(filepath):
    """Parse Exp1 log file with thread count."""
    filename = os.path.basename(filepath)

    # Parse filename: Exp1_Thr40_Mem2GB.log
    match = re.search(r'Exp1_Thr(\d+)_Mem(\d+)GB\.log', filename)
    if not match:
        return None
    ...
    with open(filepath, 'r') as f:
        ...
        # Find the [avg] line in Benchmark Results Summary
        avg_match = re.search(
            r'Exp1_Thr\d+_Mem\d+GB\[avg\]\s+(\S+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)',
            content
        )
        ...
        if avg_match:
            ...
            result['num_runs'] = int(avg_match.group(2))
            result['gen_threads'] = int(avg_match.group(3))
            result['merge_threads'] = int(avg_match.group(4))
            result['total_time'] = float(avg_match.group(5))
            result['rungen_time'] = float(avg_match.group(6))
            result['merge_time'] = float(avg_match.group(7))
            result['entries'] = int(avg_match.group(8))
...
output_path1 = os.path.join(OUTPUT_DIR, 'lineitem_exp6_exp1_total_time_heatmap.png')
...
output_path2 = os.path.join(OUTPUT_DIR, 'lineitem_exp6_exp1_total_io_heatmap.png')
...
fig4, ax4 = create_heatmap(
    time_efficiency,
    'Exp6 vs Exp1: Time Efficiency (% of Optimal)',
    'RdYlGn',
    'Efficiency (%)',
    vmin=0, vmax=100,
    format_str='.0f',
...
output_path4 = os.path.join(OUTPUT_DIR, 'lineitem_exp6_exp1_time_efficiency_heatmap.png')
...
fig5, ax5 = create_heatmap(
    io_efficiency,
    'Exp6 vs Exp1: I/O Efficiency (% of Optimal)',
    'RdYlGn',
    'Efficiency (%)',
    vmin=0, vmax=100,
    format_str='.0f',
...
output_path5 = os.path.join(OUTPUT_DIR, 'lineitem_exp6_exp1_io_efficiency_heatmap.png')
...
fig6, ax6 = create_heatmap(
    ...
    'Exp6 vs Exp1: Throughput Efficiency (% of Optimal)',
    'RdYlGn',
    'Efficiency (%)',
    vmin=0, vmax=100,
    format_str='.0f',
...
output_path6 = os.path.join(OUTPUT_DIR, 'lineitem_exp6_exp1_throughput_efficiency_heatmap.png')
...
fig7, ax7 = create_binned_heatmap(
    time_efficiency,
    'Exp6 vs Exp1: Time Efficiency - Binned (Performance Tiers)',
    'Efficiency Tier',
    format_str='.0f'
)
output_path7 = os.path.join(OUTPUT_DIR, 'lineitem_exp6_exp1_time_efficiency_binned.png')
...
fig8, ax8 = create_binned_heatmap(
    io_efficiency,
    'Exp6 vs Exp1: I/O Efficiency - Binned (Performance Tiers)',
    'Efficiency Tier',
    format_str='.0f'
)
output_path8 = os.path.join(OUTPUT_DIR, 'lineitem_exp6_exp1_io_efficiency_binned.png')
...
fig9, ax9 = create_binned_heatmap(
    throughput_efficiency,
    'Exp6 vs Exp1: Throughput Efficiency - Binned (Performance Tiers)',
    'Efficiency Tier',
    format_str='.0f'
)
output_path9 = os.path.join(OUTPUT_DIR, 'lineitem_exp6_exp1_throughput_efficiency_binned.png')
